...
FNT::FNT(vector<u8 *> letters, u8 width, u8 height, u8 low) {
	this->close();
...
void FNT::open_data(const u8 *buffer, int size) {
	FNT_HEADER header;
	...
	for (int n = 0; n <= header.highIndex-header.lowIndex; n++) {
		...
		u8 *letter = new u8[header.maxHeight*header.maxWidth];
...
BMP* FNT::export_data(Palette *palette) {
	...
	Pixels pixels(fullWidth,this->height);
	for (int i = 0; i < 256-this->lowIndex; i++) {
		Pixels image = this->get_letter(i+this->lowIndex);
		for (int y = 0; y < this->height; y++) {
			memcpy(pixels.pixels+(fullWidth*y)+(this->width*i), image.pixels+(this->width*y), this->width);
		}
	}
		...
		if ((*dim).width) {
			...
			for (int y = (*dim).yOffset; y < (*dim).yOffset+(*dim).height; y++) {
				for (int x = (*dim).xOffset; x < (*dim).xOffset+(*dim).width; x++) {
					u8 color = (*letter)[x+y*this->width];
					if (color == 0 && jmp < 31) {
						jmp++;
					} else {
...
Pixels FNT::get_letter(u8 n) {
	...
	Pixels pixels(this->letters[n-lowIndex], this->width, this->height);
	return pixels;
}
...
FNT_CHR_HEADER FNT::letter_dims(Pixels letter) {
	...
	dims.width = 0;
	dims.height = 0;
	dims.xOffset = letter.width;
	dims.yOffset = letter.height;
	for (int y = 0; y < letter.height; y++) {
		bool empty = true;
		for (int x = 0; x < letter.width; x++) {
			char c = letter.pixels[x+y*letter.width];
			if (c != 0) {
				if (x < dims.xOffset)
					dims.xOffset = x;
				if (x + 1 > dims.width)
