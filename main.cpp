...
//	ai2.import_file("test.txt");
	...
	int groups = (int)ceil(grp->get_frames() / 17.0);
	int wide = min(17,grp->get_frames());
	Pixels marine(wide*grp->get_width(), groups*grp->get_height());
	for (int g = 0; g < groups; g++) {
		int y = g * grp->get_height();
		wide = min(17,grp->get_frames()-17*g);
		for (int f = 0; f < wide; f++) {
			int x = f * grp->get_width();
			marine.paste(grp->get_frame(g*17+f), x, y);
		}
	}
	bmp->set_pixels(marine);
	bmp->save_file("marine.bmp");
...
//	}
	...
	Pixels map(dim->get_width()*32, dim->get_height()*32);
	for (int y = 0; y < dim->get_height(); y++) {
		int py = y * 32;
		for (int x = 0; x < dim->get_width(); x++) {
			int px = x * 32;
